...
async def on_message(message):
    if message.content.upper().startswith('+INFO'):
        embed = discord.Embed(title="BotDev", description="Thanks for using BotDevs! Below is info about me.", color=0x0011fa)
        embed.add_field(name="How was I made?", value="I was made in python 3.5 using discord.py", inline=False)
        embed.add_field(name="Who made me?", value="A person called Savage#1636 Made Me", inline=False)
    ...
    if message.content.upper().startswith('+COMMANDS'):
        embed = discord.Embed(title="BotDevs", description="Thanks for using BotDevs! Below are my commands. None of these are case sensetive.", color=0x0011fa)
        embed.add_field(name="+Info", value="gives info about BotDev!", inline=False)
    ...
    if message.content.upper().startswith('+VERSION'):
        embed = discord.Embed(title="BotDevs Version", descripton="Here is my version below.", color=0x0011fa)
        embed.add_field(name="version 2.7.2", value="adding +invite command", inline=False)
        await client.send_message(message.channel, embed=embed)

    if message.content.upper().startswith('+ONLINE'):
        embed=discord.Embed(title="BotDevs", color=0x00f510)
        embed.add_field(name="BotDev At The Moment:", value="Online!", inline=False)
        await client.send_message(message.channel, embed=embed)

    if message.content.upper().startswith('+OFFLINE'):
        embed=discord.Embed(title="BotDevs", color=0xf50004)
        embed.add_field(name="BotDevs Status At The Moment:", value="Offline", inline=False)
        await client.send_message(message.channel, embed=embed)
    ...
    if message.content.upper().startswith('<@459175543154147328>'):
        await client.send_message(message.channel, "HEY WHY U PINGING OWNER HE IS BUSY DEVELOPING COMMANDS FOR ME")
    if message.content.upper().startswith('+SERVERS'):
        await client.send_message(message.channel, "I Am In 1 Server At The Moment")
        
#bot token
      
client.run("NTIyNDI5NTc2NzI3NzU2ODE5.DvK2nw.zqdPZG8z8i_6_-3uLj_ge1RdlH4")
